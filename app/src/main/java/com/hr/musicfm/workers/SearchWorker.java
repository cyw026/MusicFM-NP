...
import com.hr.musicfm.extractor.search.SearchEngine;
import com.hr.musicfm.extractor.search.SearchResult;
...
public class SearchWorker extends AbstractWorker {
    ...
    protected void doWork(int serviceId) throws Exception {
        ...
        String searchLanguage = sharedPreferences.getString(searchLanguageKey, getContext().getString(com.hr.musicfm.R.string.default_language_value));
    ...
    protected void handleException(final Exception exception, int serviceId) {
        ...
        if (exception instanceof ReCaptchaException) {
            ...
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onSearchError(com.hr.musicfm.R.string.network_error);
                }
            });
            ...
            ErrorActivity.reportError(getHandler(), getContext(), exception, null, rootView, ErrorActivity.ErrorInfo.make(UserAction.SEARCHED, getServiceName(), query, R.string.parsing_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onSearchError(com.hr.musicfm.R.string.parsing_error);
                }
            });
        } else {
            View rootView = getContext() instanceof Activity ? ((Activity) getContext()).findViewById(android.R.id.content) : null;
            ErrorActivity.reportError(getHandler(), getContext(), exception, null, rootView, ErrorActivity.ErrorInfo.make(UserAction.SEARCHED, getServiceName(), query, com.hr.musicfm.R.string.general_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
