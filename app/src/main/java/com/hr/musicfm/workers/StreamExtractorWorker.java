...
import com.hr.musicfm.extractor.exceptions.ReCaptchaException;
import com.hr.musicfm.extractor.services.youtube.YoutubeStreamExtractor;
import com.hr.musicfm.extractor.stream_info.StreamExtractor;
...
public class StreamExtractorWorker extends ExtractorWorker {
    ...
    protected void doWork(int serviceId, String url) throws Exception {
        ...
        if (streamInfo != null && !streamInfo.errors.isEmpty()) handleErrorsDuringExtraction(streamInfo.errors, UserAction.REQUESTED_STREAM);
    ...
    protected void handleException(final Exception exception, int serviceId, final String url) {
        ...
        if (exception instanceof ReCaptchaException) {
            ...
            ErrorActivity.reportError(getHandler(), getContext(), exception, MainActivity.class, null, ErrorActivity.ErrorInfo.make(UserAction.REQUESTED_STREAM, getServiceName(), url, R.string.youtube_signature_decryption_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
            ...
            if (!streamInfo.errors.isEmpty()) {
                ...
                ErrorActivity.reportError(getHandler(), getContext(), exception, MainActivity.class, null, ErrorActivity.ErrorInfo.make(UserAction.REQUESTED_STREAM, getServiceName(), url, R.string.could_not_get_stream));
            } else {
                ErrorActivity.reportError(getHandler(), getContext(), streamInfo.errors, MainActivity.class, null, ErrorActivity.ErrorInfo.make(UserAction.REQUESTED_STREAM, getServiceName(), url, com.hr.musicfm.R.string.could_not_get_stream));
            }
            ...
            ErrorActivity.reportError(getHandler(), getContext(), exception, MainActivity.class, null, ErrorActivity.ErrorInfo.make(UserAction.REQUESTED_STREAM, getServiceName(), url, R.string.parsing_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onUnrecoverableError(exception);
            ...
            ErrorActivity.reportError(getHandler(), getContext(), exception, MainActivity.class, null, ErrorActivity.ErrorInfo.make(UserAction.REQUESTED_STREAM, getServiceName(), url, R.string.general_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onUnrecoverableError(exception);
