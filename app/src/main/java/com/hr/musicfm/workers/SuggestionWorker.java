...
public class SuggestionWorker extends AbstractWorker {
    ...
    protected void handleException(final Exception exception, int serviceId) {
        ...
        if (exception instanceof ExtractionException) {
            ...
            ErrorActivity.reportError(getHandler(), getContext(), exception, null, rootView, ErrorActivity.ErrorInfo.make(UserAction.GET_SUGGESTIONS, getServiceName(), query, R.string.parsing_error));
            ...
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onSuggestionError(com.hr.musicfm.R.string.network_error);
                }
            });
        } else {
            View rootView = getContext() instanceof Activity ? ((Activity) getContext()).findViewById(android.R.id.content) : null;
            ErrorActivity.reportError(getHandler(), getContext(), exception, null, rootView, ErrorActivity.ErrorInfo.make(UserAction.GET_SUGGESTIONS, getServiceName(), query, R.string.general_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
